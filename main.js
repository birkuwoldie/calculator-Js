class Calculator{
    ...
    getDisplayNumber(number){
        ...
        if (isNaN(integerDigits)){
            ...
            intrgerDisplay = integerDigits.toLocaleString('en',{maximumFractionDigits: 0})
        }
        if (decimalDigits != null){
            return `${intrgerDisplay}.${decimalDigits}`
        }
        else{
            return intrgerDisplay
        }   
    }
    updateDisplay() {
        ...
        if (this.operation != null) {
          this.previousOperandandTextElement.innerText = `${this.getDisplayNumber(this.previousOperand)} ${this.operation} ${this.currentOperand !== '' ? this.currentOperand : ''}`;
        } else {
          this.previousOperandandTextElement.innerText = this.getDisplayNumber(this.previousOperand);
        }
      }
